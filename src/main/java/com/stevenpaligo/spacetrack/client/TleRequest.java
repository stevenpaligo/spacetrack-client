...
public class TleRequest {
  ...
  @Singular
  private Collection<Predicate<TleFields>> predicates;

  private Limit limit;

  @Singular
  private List<Sort<TleFields>> sorts;
  ...
  public List<Tle> execute() throws JsonParseException, JsonMappingException, IOException {
    ...
    // create a request
    Request<TleFields, Tle> request = new Request<>(credentials, "tle", CollectionUtils.emptyIfNull(predicates), Optional.ofNullable(limit), ListUtils.emptyIfNull(sorts), SetUtils.emptyIfNull(favorites));
